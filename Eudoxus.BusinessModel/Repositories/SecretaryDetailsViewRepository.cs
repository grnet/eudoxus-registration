...
{
    public class SecretaryDetailsViewRepository : DomainRepository<HelpDeskViewsEntities, SecretaryDetailsView, int>
    {
        #region [ Base .ctors ]

        public SecretaryDetailsViewRepository() : base() { }

        public SecretaryDetailsViewRepository(Imis.Domain.IUnitOfWork uow) : base(uow) { }

        #endregion

        public List<SecretaryDetailsView> FindSecretariesWithCriteria(Criteria<SecretaryDetailsView> criteria, out int totalRecordCount)
        {
            var query = BaseQuery;
