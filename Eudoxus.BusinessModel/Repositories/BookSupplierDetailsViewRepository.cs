...
{
    public class BookSupplierDetailsViewRepository : DomainRepository<HelpDeskViewsEntities, BookSupplierDetailsView, int>
    {
        #region [ Base .ctors ]

        public BookSupplierDetailsViewRepository() : base() { }

        public BookSupplierDetailsViewRepository(Imis.Domain.IUnitOfWork uow) : base(uow) { }

        #endregion

        public List<BookSupplierDetailsView> FindBookSuppliersWithCriteria(Criteria<BookSupplierDetailsView> criteria, out int totalRecordCount)
        {
            var query = BaseQuery;
